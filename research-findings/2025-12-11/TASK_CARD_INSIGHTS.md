...
> ⚠️ **Data Note:** The `task_card_viewed` event was first deployed on December 11, 2025 at 12:39 PM PST. Early metrics reflect ~8 hours of data collection. These numbers will become more representative after 2-4 weeks.
>
...
1. **"What is the average number of task_card_viewed events per user? Show the distribution of how many task cards each user opens."**

2. **"Show me the total task_card_viewed events all time, grouped by task_category. Which categories get the most views?"**

3. **"What percentage of users who view a task card (task_card_viewed) go on to claim a task (task_claimed)? Show conversion rate."**

4. **"When was the first task_card_viewed event ever recorded? Show the earliest timestamp."**

5. **"How many total unique users (distinct person_id) have ever triggered any event? Compare to how many have triggered task_card_viewed."**

6. **"Of users active today, how many visited the gallery (gallery_viewed event) vs how many opened a task card (task_card_viewed)?"**

---

## PostHog Queries Generated

### 1. When was the first task_card_viewed event?

```sql
SELECT min(timestamp) AS first_task_card_viewed_timestamp
FROM events
WHERE event = 'task_card_viewed'
```

### 2. Total platform users vs task card viewers

```sql
SELECT
    count(DISTINCT person_id) AS total_users,
    countIf(DISTINCT person_id, event = 'task_card_viewed') AS task_card_viewed_users
FROM events
```

### 3. Gallery to task card conversion (daily unique users)
...
### 4. Average views per user with distribution
...
### 5. View to claim funnel (conversion rate and time to claim)
